...
- Remediation: the post-review worktree implements and locally verifies all three release blockers, Phase 2 findings M1-M9, and Phase 3 findings M10-M19 (OpenAPI); public-documentation reconciliation is partially complete (see "Remediation Progress" and "Phase 3 Remediation And Review")
- Second pass: an independent adversarial review of the Phase 1/2 remediation diff confirmed nine defects in it (one blocker, eight major), all fixed in-tree and re-verified; a follow-up review of those repairs confirmed three more (all in the error-formatting paths), also fixed (see "Second-Pass Review And Repairs")
- Phase 3: implemented as seven ordered OpenAPI clusters on 2026-08-10; an eight-dimension adversarial review of that diff confirmed nine defects, six fixed in-tree (id-kind seeds, enveloped create, two SSRF host-check gaps, a seed-path over-rejection, allow-list case) and three deferred with recorded reasons (see "Phase 3 Remediation And Review")
- Phase 4: implemented as five conflict-ordered adapter/CLI clusters on 2026-08-10; a six-dimension adversarial review of that diff verified fifteen findings, confirmed twelve (ten distinct defects, two majors among them), refuted three, and all ten were fixed in-tree (see "Phase 4 Remediation And Review")
- Phase 5: implemented as three waves on 2026-08-10 covering the data plugins (M20-M23), all remaining minor findings, the documentation-drift rows, and the packaging/dead-code/CI items — 70 work items, 15 more found already closed; an eight-dimension adversarial review confirmed 20 defects and refuted 5, 19 were fixed in-tree and one (Stripe's tallest response schemas) is deferred with measurements on the record (see "Phase 5 Remediation And Review")
...
| Test boundary coverage | All five phases' boundaries repaired and pinned; package READMEs remain the one shipped surface with no gate |
| Packaging and declaration integrity | Release-blocking at the reviewed SHA; remediation is green locally |
| Public documentation | All drift rows reconciled; `docs/getting-started.md` snippets now execute in the integration suite |
| Security defaults | Safe locally, unsafe when explicitly exposed |
| Immediate recommendation | Run fresh CI on the completed five-phase remediation, then decide on graceful truncation for very tall schemas |
...
| Finding | Status | Repair and regression coverage |
|---|---|---|
...
| Severity | Defect | Repair |
|---|---|---|
| Major | `generateSeedItems` stamped an integer id on every seeded row regardless of the resource's id kind, so a `type: string`/`format: uuid` resource carried integer seeds and string/uuid creates in one collection, 500-ing under `validateResponses`. | Seeded ids are shaped by the same `shapeId` the mint path uses, and `createSeeder` recovers the counter from each id kind (synthetic uuids included) so creation resumes past the seed max without collision. |
| Major | An enveloped create response (`{ data: <Resource> }`) stored a fabricated resource under `data` plus a stray id at the root, so `GET /x/{returned-id}` 404-ed; a closed envelope 500-ed. | Create now builds and stores the bare resource from the item schema and wraps it in the declared envelope only for the response, injecting the real item into the wrapper slot. |
| Major | `isUnsafeHost` missed IPv4-mapped IPv6 literals (`[::ffff:169.254.169.254]`) and trailing-dot hosts, defeating the only SSRF control in the documented empty-`allowedHosts` mode. | The check sees through IPv4-mapped IPv6 to the embedded address and strips a trailing dot before the name comparisons. |
| Major | `readHttpRef` followed redirects internally and never re-checked the target, so an allow-listed host could bounce a `$ref` to a loopback/RFC1918 address. | Redirects are followed manually with a per-hop `checkRef`, a whole-chain timeout, and a bounded hop count; a blocked hop is never fetched. |
| Major | The seed-manifest boundary check used `rel.startsWith("..")`, refusing in-directory files whose name begins with `..` — including the Kubernetes ConfigMap layout — so the server would not boot. | The check rejects only an exact `..` first segment (`..`, `../…`) plus absolute and self paths. |
| Minor | `allowedHosts` compared case-sensitively against the always-lowercased `URL.hostname`, so an allow-list entry with any uppercase letter silently blocked every ref. | Allow-list entries are lowercased when the policy is resolved. |

Deferred with recorded reasons (not fixed in Phase 3):

- **Request-path generation is unbounded (perf).** Every create generates the whole declared response contract, and `@schmock/faker`'s `alwaysFakeOptionals` materializes every optional property, so a very wide create-response schema costs seconds and a large body per `POST`. The node budget does not bind it — the cost is schema *width*, not node count, and the generated body stays far under the node limit. Deferred to Phase 5 (faker is in scope there); the per-request opt-out is `onSchema` returning a trimmed schema for the create method, now documented. The false in-tree comment claiming a budget bounds this path was corrected.
- **Per-scope state is not evicted.** M13's per-parent isolation allocates collection/counter/seed state for every distinct parent id ever requested, with no cap, so a long-running server hit across a very large parent-id range grows state unboundedly. Per-parent isolation genuinely requires per-parent state; a cap or LRU is a new mechanism, deferred and documented as a limitation.
- **`readHttpRef` diagnostics are laundered.** Its size/timeout/status error messages collapse to ref-parser's opaque `Error reading file "<url>"` because it throws a plain `Error` rather than the library's `ResolverError`. The proper fix needs the `@apidevtools/json-schema-ref-parser` type import that was just removed from `ref-policy.ts` to keep `check:publish` green (naming the library's Node-typed declarations in an exported signature breaks consumers without `@types/node`); deferred rather than reintroduce that coupling.

Three review findings were refuted and left unchanged: that `applyOverrides` destroys a list's per-media-type contract on a no-op override (the `dropMediaTypeContracts` clear is what keeps the override path working and is pinned green); that `onSchema` never fires on the create path for incomplete specs (it fires at the contract base, verified); and that two of the `incomplete-schema` 200→500 flips regress legal specs (the old `200 {}` was the laundered failure the cluster exists to remove).

## Phase 4 Remediation And Review (Adapters And Exposed Server)

Phase 4 was implemented in this worktree on 2026-08-10 as five conflict-ordered clusters over the Angular and Express adapters, the core interception registry, the React and Vue providers, and the CLI server. A scout pass first re-verified every M24-M29 sub-claim against the current tree — several were already closed by the Phase 1/2 second pass (Angular non-2xx channel, HEAD body stripping, the formatter provenance guards' first half, framing-header stripping of `Content-Length`/`Transfer-Encoding`) — and found one defect the baseline missed: both adapters read the exception-provenance marker only *after* user hooks, so a documented spread-style `beforeResponse`/`transformResponse` silently disabled `errorFormatter`. The findings below still describe revision `3f64677`; this section records the delta.

| Finding | Status | Repair and regression coverage |
|---|---|---|
| M24 | Implemented (breaking) | Request header names are lowercased at the single `mock.handle` boundary, so `transformRequest`-supplied capitalized keys fold too. Bodies are shaped to `HttpRequest.responseType` on every emit path (success, error channel, strict-mode 404, catch-path 500) using Angular-faithful empty-body semantics: `null` only at 204, otherwise `''`/empty `ArrayBuffer`/empty `Blob` for a bodyless status, with `HttpErrorResponse`'s own falsy-to-null coercion left intact. Emitted responses report `req.urlWithParams`. `transformRequest` and request derivation now run per subscription inside the Observable behind one guarded error path, and every exit returns the teardown (the unsupported-method branch no longer leaks its inner subscription). |
| M25 | Implemented | Core exports `getResponseException`; both adapters read provenance from the raw response *before* user hooks, so response-cloning hooks can no longer suppress the formatter. The Express formatter is additionally gated on the post-hook status staying 500 — a hook that rewrites an exception to 503/200 is honored — and the formatted 500 carries the post-hook headers with `content-type` forced to JSON, retrying once with the fixed header set when an inherited header is untransportable rather than discarding the formatted body. Formatters fire exactly once; admission acquisition sits inside the try so a malformed mock takes the normal error path instead of rejecting the middleware promise. |
| M26 | Implemented (breaking) | A mock now holds N concurrent interception leases in a `Set`; `ALREADY_INTERCEPTING` is retired. `InterceptHandle` gains a required `update(options?)` that swaps options in place, preserving registry position. Per-request owner de-duplication makes N leases on one mock consult `handle()` once (the `baseUrl` filter deliberately does not mark the owner, so disjoint-baseUrl leases each get their chance). React splits lease acquisition from option updates across two layout effects; Vue tracks leases per app in a `WeakMap`, releases on unmount and on a throwing `mount`, exports `restoreSchmockInterception(app)`, and skips the fetch patch without a DOM while still providing the mock (React's installer applies the same SSR rule). |
| M27 | Implemented (breaking) | The admin API is authenticated: `Authorization: Bearer` or `x-schmock-admin-token`, a token auto-minted and printed to stderr when `--admin-token` is absent, 401 with `www-authenticate` otherwise, constant-time comparison, and admin-disabled paths still 404. Any `Origin`-bearing admin request is refused 403; the admin surface carries no CORS headers on any path, including the handler's catch path (the admin check is computed before method parsing so a rejected verb still answers without CORS, while dispatch order — and the 405 — are unchanged). The history projection redacts `authorization`, `proxy-authorization`, `cookie`, `set-cookie`, `x-api-key`, `x-auth-token`, and `x-schmock-admin-token`; history is capped (default 500, `--admin-history-limit`) and disabled entirely without `--admin`. Non-loopback admin binds warn. |
| M28 | Implemented (breaking) | One HTTP server lives for the whole process; a reload swaps the mock behind the socket, so active connections survive and a failed reload leaves the old mock serving. `createCliServer` owns the watcher, honors `watch: true`, and rejects with an unbound port if the watcher cannot be created. `close()` returns a memoized `Promise<void>` that stops accepting *first* and bounds the whole shutdown — including a watcher drain blocked on an in-flight spec parse — by `shutdownGraceMs` (default 5000). Signal handlers are released only after the close settles, so a second Ctrl-C is absorbed by the re-entrant guard instead of restoring the default disposition and hard-killing the drain. |
| M29 | Implemented (breaking) | CORS answers 204 only to a true preflight (`OPTIONS` + `Origin` + `Access-Control-Request-Method`); a spec-declared OPTIONS operation is reachable, an unrouted OPTIONS 404s, and `Access-Control-Allow-Headers` echoes the requested headers. The response normalizer strips hop-by-hop headers (`connection`, `keep-alive`, `proxy-authenticate`, `proxy-authorization`, `te`, `upgrade`) unconditionally beside the existing framing-header stripping; the ingress-error `connection: close` is re-added at write time via `extraHeaders` on every error write path so it survives its own stripping. HEAD/304 keeping a declared `Content-Length` remains deliberate and pinned. |

A subsequent adversarial review (Claude, 2026-08-10) put the Phase 4 diff through six dimension reviewers with per-finding refutation: fifteen findings were verified, twelve confirmed (ten distinct defects — two of the confirmations duplicated the close-during-reload defect and the admin-preflight coverage gap across dimensions), three refuted. All ten were fixed in this worktree:

| Severity | Defect | Repair |
|---|---|---|
| Major | `close()` awaited the watcher drain — an unbounded in-flight spec reload — before ever releasing the socket, so the server kept accepting throughout and `shutdownGraceMs` bounded nothing, contradicting the declared contract in three places. | `close()` starts the socket close immediately and races the cleanup drain against the same grace bound; an overrunning drain is abandoned (its observable effects are already suppressed by the watcher's `closed` flag). Regression test parks a reload on a gated spec parse and asserts the port refuses connections at once and the close settles within the bound. |
| Major | Error responses on admin paths carried wildcard CORS headers under `--cors`: the admin-path check lived after method parsing, so a rejected verb reached the catch block with the admin flag unknown. | The admin-path boolean is computed before the method parse (dispatch order unchanged, the 405 preserved) and the catch's CORS merge is gated on it; a BDD step pins the error path. |
| Minor | A second SIGINT during the grace window hard-killed the process: both listeners were removed at the start of shutdown, restoring the default disposition, so the documented re-entrant guard was unreachable for a real signal. | Listeners stay attached until the close settles and are released on the settle path; the guard now absorbs the repeat signal. |
| Minor | The admin CORS-preflight exclusion had no discriminating coverage: the scenario's OPTIONS sent neither `Origin` nor `Access-Control-Request-Method`, so removing the exclusion left every admin CORS assertion green. | The scenario now sends a real preflight and pins 403 + `FORBIDDEN` + no CORS headers, alongside separate bare-OPTIONS and unsupported-method pins. |
| Minor | The history projection redacted six credential headers but not `x-schmock-admin-token` — the one header this file itself mints as a credential — so a pasted history dump contained a live token. | Added to the mask, the docs enumeration, and the redaction scenario. |
| Minor | The `denyAdminRequest` comment claimed the Origin check blocks DNS rebinding; a rebound page is same-origin and sends no `Origin` — the token is what stops it. A maintainer trusting the comment could relax the token believing rebinding stayed covered. | Comment corrected to attribute the rebinding defence to the bearer token. |
| Minor | Angular emitted `null` for empty bodies under `responseType` text/arraybuffer/blob at every status, where the real `HttpXhrBackend` nulls only 204 and otherwise yields `''`/empty buffers — and a test pinned the divergence. | Status-aware empty representation (204 stays `null`); the pin was flipped, seven cases added across HEAD/205/304/`[200, null]`, and the doc bullets reconciled. Naively deleting the guard would have serialized `null` to the string `"null"` — avoided. |
| Minor | The formatted Express 500 inheriting post-hook headers regressed when a hook supplied an untransportable header (non-string, control character, case-duplicate): the normalizer's throw diverted the response to the generic fallback body, silently discarding the formatter output. | The inherited-header send retries once with the fixed JSON header set, keeping the already-captured formatted body; the formatter still fires exactly once. Three red-then-green tests pin it. |
| Minor | One mock holding N leases (the newly blessed nested-provider pattern) ran `mock.handle()` once per lease for a single unmatched fetch, double-emitting `request:start`/`notfound`/`end` for one network call. | Each lease carries a per-mock owner symbol and the dispatch loop skips further leases of an owner whose handler already ran — keyed on handler-ran, not on PASSTHROUGH, so the `baseUrl` filter never marks the owner and disjoint-baseUrl leases still compose. Both behaviors are pinned. |
| Minor | A stale comment in the CLI integration test claimed only the concrete `CliServer` type carries the awaitable `close()`; the ambient type gained it in this same diff. | Comment corrected to the true remaining reason (the concrete type keeps `server` as the real `http.Server`). |

The three refuted findings were left unchanged: a claimed Angular header/body content-type contradiction on the formatter path (the contradiction exists in source but is unreachable — the only producer of exception-marked responses is core's catch block, which always attaches a JSON content-type, and `markResponseException` is not public); a claimed HEAD/GET asymmetry on admin endpoints (real, but it is the library's routing model — HEAD is a first-class method never derived from GET, and mock routes behave identically); and a claimed documentation lie about the hop-by-hop stripping (the docs make an affirmative framing-header statement with no exhaustiveness or pass-through promise anywhere, so nothing became false).
...
| Finding | Status | Repair and regression coverage |
|---|---|---|
| M20 | Implemented (breaking) | One cycle-aware walker now validates the whole schema, descending into untyped nodes and checking `faker` method names at every depth instead of only the top level; a genuine cycle throws from validation rather than being silently generated. `minItems` above the array cap throws at construction instead of at generation time. Smart mapping never replaces a `format` json-schema-faker can satisfy and skips schemas with numeric constraints it would violate; the deferral set is pinned by a guard test that generates every member. json-schema-faker's `maxDepth` is set explicitly — its default of 5 silently emitted `{}`/`null` past that depth, so a schema this package advertised as generatable produced a body that violated its own `required` contract. |
| M21 | Implemented (breaking) | Seeded generation anchors faker's relative date methods to a fixed reference date, so `date.recent`/`date.future` reproduce across runs; unseeded generation stays wall-clock relative. Generated values are deep-cloned on the way out, so two array items built from one shared sub-schema are no longer aliased and a literal `__proto__` key in a schema default stays data. The json-schema-faker format registry is module-global, so Schmock now starts each generation from the built-in formats only — a consumer's own `registerFormat` no longer changes Schmock's output, and Schmock's no longer changes theirs. |
...
| Severity | Defect | Repair |
|---|---|---|
| Critical | Raising json-schema-faker's depth and making the schema walker measure nesting accurately were landed as separate items, but the depth *ceiling* had been calibrated against the old undercounting walker. Together they regressed twelve Stripe GET routes from a 200 to a `RESOURCE_LIMIT_ERROR` 500. | Deferred after measurement — see "Stripe's tallest response schemas" below. The two constants were recalibrated against the repo's own fixtures (`MAX_NESTING_DEPTH` 10 to 15, `JSF_MAX_DEPTH` from an unmoored 24 to `MAX_NESTING_DEPTH + 5`), which restores three routes' declared list envelopes and quarters recursive `$ref` output, but does not restore the twelve. |
| Major | Raising `maxDepth` to 24 unbounded recursive `$ref` generation: the schema-node and generated-node budgets never resolve `$ref`, so a self-referencing `$defs` subtree was bounded by nothing. A comment claimed the budgets covered it. | `JSF_MAX_DEPTH` is now derived from the ceiling rather than chosen independently, and the comment says what is true — this option is the only bound on generated depth, and why. Measured effect on a recursive `$defs` chain: 18.5 MB in 1376 ms becomes 3.1 MB in 144 ms. |
...
| Major | The Angular package README wired the interceptor with a bare `provideHttpClient()`, which silently never intercepts. | `provideHttpClient(withInterceptorsFromDi())`, matching `docs/angular.md`, typechecked against the built declarations a consumer actually resolves. |
| Major | The Faker package README's only usage example did not compile and implied a route-scoped `.pipe()` that does not exist. | Split into `mock.pipe(fakerPlugin(...))` and `mock("GET /users", undefined)`, verified by both typecheck and a real request returning three seeded users. |
| Minor | `GENERATABLE_FORMATS` listed `uri-template` and `regex`, which json-schema-faker cannot generate, so smart mapping was suppressed in favour of a random alphanumeric string (or, at some seeds, an empty one). | Both removed; the set now equals json-schema-faker 0.6.2's built-in registry. A guard test generates every remaining member at four seeds, so the set cannot drift on an upgrade. |
| Minor | "Own enumerable properties only" was false in both plugins: a non-enumerable own property satisfies `required` and can be filtered on, while `JSON.stringify` omits it from the wire. | Both sentences corrected to "own properties only, enumerable or not", each verified by probe. The mechanism is sanctioned and unchanged. |
| Minor | Two of the five new query own-property tests passed identically without the own-property read — the key they looked up was not on the prototype. | Rewritten with `Object.create({...})` so the inherited key exists; discrimination proven both ways by temporarily reverting the read. |
| Minor | The request-spy path filters compared raw strings, so the exact path passed to `handle()` stopped matching its own history record once the write side canonicalized. | One private `#historyMatcher` canonicalizes the filter the same way the write side does, shared by `history()`, `called()`, `callCount()` and `lastRequest()`. The documented caveat that history stores the namespace-stripped path was added after a probe. |
| Minor | Pre-loaded CRUD collection state did not reset the id counter, so the first POST minted an id that already existed. | The create path recovers the high-water mark from the live collection when no numeric counter is stored, mirroring the seeder, using the non-materializing read. A stored counter stays authoritative even at 0. |
| Minor | The CLI README claimed a random free port that the documented invocation never produced. | Documented the bare form's default of 3000 and added an explicit `--port 0` example, after confirming the banner reports the OS-assigned port. |
| Minor | Every tarball declared MIT but shipped no LICENSE file, and the new manifest checks assumed one was there. | `check-manifests` now fails on a missing per-package LICENSE and on any drift from the root LICENSE; eleven byte-identical copies added. Both branches mutation-probed, and the result confirmed in tarballs from both packers. |
| Minor | `schmock.d.ts` still documented a schema generator arm that the same file removed. | The one stale `@param` line replaced; core rebuilt so the published declarations carry it. |

The five refuted findings were left unchanged: a claim that `$defs`/`definitions`/`not`/`if`/`then`/`else` wrongly contribute to the nesting estimate (they are counted deliberately, and the walker's height rules are sanctioned); a claim that the constrained-numeric skip's comment overstates its coverage for type-less numeric schemas; and three packaging claims — twice that the narrowed `files` set ships declaration maps pointing at unpublished sources, and once that the integration runner still reports success for a fixture running zero tests, all of which the wave had already closed.

### Stripe's tallest response schemas

The critical finding is the one item this phase does not close, so the measurements are recorded here rather than in a follow-up.

`walkSchema` stops descending at the ceiling, so a breach always reports `actual` = ceiling + 1. The value that made a 15 look sufficient was that saturation artifact, not a measurement. Measured with an uncapped mirror of the walker's height rules, cross-checked against the real validator over 518 schemas where no other limit fired, the dereferenced and normalized response schemas measure: petstore 2, openapi31 2, train-travel 3, scalar-galaxy 6, and Stripe up to 47. Stripe's 1,176 response schemas are not a continuum — they land on 0-13 or on 28-47 with nothing in between — and every one of the eighteen Stripe GET routes that 5xx has a schema of height 30-40. Every ceiling from 14 to 27 therefore admits exactly the same set, which is why 15 restores none of them.

Admitting the tall cluster is not affordable. Measured with validation bypassed at the current json-schema-faker depth, one response costs 97 MB (`GET /v1/setup_attempts`), 127 MB (`/v1/invoices/:id/lines`) and 188 MB (`/v1/checkout/sessions/:id/line_items`).

What HEAD returned for these routes was generated at json-schema-faker's default depth of 5, and on at least some of them that was not a correct body. `GET /v1/setup_attempts` produced 513 KB reaching six levels, with at least six `required` properties emitted as `{}` where the spec declares a structured object — for instance `data[0].payment_method_details.bancontact.generated_sepa_debit.billing_details`; the same schema at the current bound produces 5.46 MB with none of those stubs. The same probe on `GET /v1/account` found no stubbed required properties, so the truncation is not uniform across the twelve. On the routes measured, then, the change is from a silently truncated 200 to an explicit 500 rather than from a working response to a broken one — the better of the two available behaviours, and still worse than what a user wants.

Two consequences are worth carrying forward. The diagnostic got less useful: twelve of the eighteen 5xx routes converted from a depth failure to a `schema_nodes` failure (limit 50,000), and that counter charges a shared sub-schema once per path to it, so Stripe's dereferenced DAG is charged roughly 74 times its distinct node count — a spec with a few hundred distinct nodes is told it blew a 50,000 budget. And a `$defs`-mediated recursive schema is still accepted while the semantically identical `$ref: "#"` is rejected as unsupported; output is four times smaller after the recalibration, but nothing rejects the input.
...
The post-review remediation closes the package-artifact, core lifecycle/transport, OpenAPI ownership, Angular/Vue adapter, and CLI exposure gaps in this list. The plugin-safety gaps remain for Phase 5.
