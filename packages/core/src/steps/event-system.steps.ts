...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario('Register and trigger request:end event', ({ Given, When, Then, And }) => {
    ...
    And('I register a handler for "request:end" event', () => {
      const handler = (data: any) => {
  ...
  Scenario('Plugin registration event', ({ Given, When, Then, And }) => {
    ...
    And('I register a handler for "plugin:registered" event', () => {
      const handler = (data: any) => {
      ...
      if (!eventHandlers.has("plugin:registered")) {
        eventHandlers.set("plugin:registered", []);
      }
      eventHandlers.get("plugin:registered")!.push(handler);
