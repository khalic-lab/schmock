...
const feature = await loadFeature(
  "../../features/developer-experience.feature",
);
...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario(
    "State confusion between global and local state",
    ({ Given, When, Then, And }) => {
      Given(
        "I create a mock with global state and a local state counter",
        () => {
  ...
  Scenario(
    "Route precedence with similar paths",
    ({ Given, When, Then, And }) => {
      Given(
        "I create a mock with an exact route and a parameterized route on {string}",
        (_, basePath: string) => {
          mock = schmock();
          mock(toRouteKey("GET", `${basePath}/profile`), { type: "profile" });
          mock(toRouteKey("GET", `${basePath}/:id`), ({ params }) => ({
            type: "user",
            id: params.id,
          }));
        },
      ...
      Then(
        "the profile route should return exact match:",
        (_, docString: string) => {
          const expected = JSON.parse(docString);
          expect(responses[0].body).toEqual(expected);
        },
      ...
      And(
        "the parameterized route should match with param:",
        (_, docString: string) => {
          const expected = JSON.parse(docString);
          expect(responses[1].body).toEqual(expected);
        },
  ...
  Scenario("Plugin order affecting results", ({ Given, When, Then }) => {
    Given(
      "I create a mock with two plugins that each set a step number",
      () => {
        mock = schmock();
  ...
  Scenario(
    "Plugin expecting different context structure",
    ({ Given, When, Then }) => {
      Given(
        "I create a mock with a plugin that reads context properties",
        () => {
          mock = schmock();
          const plugin: Plugin = {
            name: "context-reader",
  ...
  Scenario("Response tuple format edge cases", ({ Given, When, Then, And }) => {
    ...
    Then(
      "the created endpoint should return status 201 with empty body",
      () => {
    ...
    And(
      "the headers endpoint should return status 200 with data and custom header",
      () => {
  ...
  Scenario(
    "Registering duplicate routes first route wins",
    ({ Given, When, Then }) => {
      Given(
        "I create a mock with two routes on {string} with different data",
        (_, route: string) => {
          mock = schmock();
  ...
  Scenario(
    "Duplicate routes are not duplicated in getRoutes()",
    ({ Given, When, Then }) => {
