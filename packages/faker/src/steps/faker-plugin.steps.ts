...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario(
    "Multiple schema plugin instances do not share state",
    ({ Given, When, Then }) => {
      let plugin1: any;
      let plugin2: any;
      let data1: any;
      ...
      Then(
        "the data from each instance should be independently generated",
        () => {
  ...
  Scenario(
    "Invalid schema is rejected at plugin creation time",
    ({ Given, Then }) => {
  ...
  Scenario(
    "Faker method validation checks actual method existence",
    ({ Given, Then }) => {
      Given(
        "I attempt to create a schema with faker method {string}",
        (_, fakerMethod: string) => {
          error = null;
          try {
            const schema = {
              type: "object" as const,
      ...
      Then(
        "it should throw a SchemaValidationError with message {string}",
        (_, message: string) => {
          expect(error).not.toBeNull();
          expect(error?.name).toBe("SchemaValidationError");
