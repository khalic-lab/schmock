...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario('Listen to request events', ({ Given, And, When, Then }) => {
    ...
    And('I register handlers for {string} and {string} events', (_, event1: string, event2: string) => {
    ...
    Then('the {string} event should be fired with:', (_, event: string, docString: string) => {
    ...
    And('the {string} event should be fired with:', (_, event: string, docString: string) => {
      const expected = JSON.parse(docString)
      const events = eventData.get(event)
      expect(events).toBeDefined()
      expect(events!.length).toBeGreaterThan(0)
  ...
  Scenario('Multiple event handlers', ({ Given, And, When, Then }) => {
    ...
    And('I register handlers {string} and {string} for {string} event in order', (_, handler1: string, handler2: string, event: string) => {
    ...
    Then('handler {string} should be called before handler {string}', (_, first: string, second: string) => {
  ...
  Scenario('Error event', ({ Given, And, When, Then }) => {
    ...
    Then('the {string} event should be fired with error {string}', (_, event: string, expectedMessage: string) => {
