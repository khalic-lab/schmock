...
schmock.use(schema({
  ...
  // Add custom formats
  formats: {
    'company-email': (name: string) => `${name.toLowerCase().replace(/\s+/g, '.')}@acme.com`
  }
}));
  ...
  '/api/orders': {
    fields: {
      ...
      status: ({ record }) => {
        ...
        if (days < 3) return faker.helpers.arrayElement(['processing', 'shipped']);
        return 'delivered';
      },
      total: ({ record, db }) => {
...
schmock.use({
  ...
  afterGenerate({ path }, data) {
    console.log(`📤 ${path} generated ${Array.isArray(data) ? data.length : 1} records`);
    return data;
  },
